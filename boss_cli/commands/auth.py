...
@click.option("--cookie-file", default=None, help="从文件导入 Cookie (credential.json 或 cookie 字符串)")
@click.option("--cookie", "cookie_string", default=None, help="直接传入 Cookie 字符串 (k=v; k2=v2)")
def login(qrcode: bool, cookie_source: str | None, cookie_file: str | None, cookie_string: str | None) -> None:
    """扫码登录 Boss 直聘 APP"""
    from ..auth import (
        clear_credential,
        load_from_cookie_file,
        load_from_cookie_string,
        save_credential,
        verify_credential,
    )
    ...
    def _finalize_login(cred, *, from_qr: bool = False) -> None:
        ...
        # Retry once if __zp_stoken__ looks stale and we have a browser source.
        if not from_qr and message and "__zp_stoken__" in message:
            from ..auth import extract_browser_credential
            console.print("[yellow]⚠️  检测到 __zp_stoken__ 失效，尝试重新读取浏览器 Cookie...[/yellow]")
            refreshed, _ = extract_browser_credential(cookie_source=cookie_source)
            if refreshed:
    ...
    if cookie_file or cookie_string:
        cred = None
        if cookie_file:
            cred = load_from_cookie_file(cookie_file)
            if not cred:
                console.print(f"[red]❌ 无法从文件加载 Cookie: {cookie_file}[/red]")
                raise SystemExit(1)
        else:
            cred = load_from_cookie_string(cookie_string or "")
            if not cred:
                console.print("[red]❌ Cookie 字符串格式不正确[/red]")
                raise SystemExit(1)
